...
PERSONALITY_CHARISMATIC = 0
PERSONALITY_UNCHARISMATIC = 1
...
def random_arm_selection():
    selected_arm = random.choice([PERSONALITY_CHARISMATIC, PERSONALITY_UNCHARISMATIC])
    print(f"Randomly selected arm: {selected_arm}")
    return selected_arm
...
def update_personality_and_context_baseline(timestamp_seconds, action, data, last_personality_change_time, last_interactivity_update_time):
    ...
    if timestamp_seconds - last_interactivity_update_time >= INTERACTIVITY_TIME_WINDOW:
        ...
        new_personality = PERSONALITY_CHARISMATIC if predicted_arm == PERSONALITY_CHARISMATIC else PERSONALITY_UNCHARISMATIC
